...
public class Voronoi
{
    ...
    public Halfedge ELhash[];
    private Halfedge ELleftend, ELrightend;
    private List<GraphEdge> allEdges;
	
    public Map<Site,Halfedge> heTable = new HashMap<Site,Halfedge>();
            ...
            double minX, double maxX, double minY, double maxY)
    {
        ...
        for(int i = 0; i < xValuesIn.length; i++) {
        	cells.add(new Cell());
        }
        ...
        heTable.clear();
        for(Halfedge he : this.ELhash) {
        	if (he == null) continue;
        	if (he.vertex != null) {
        		if (!heTable.keySet().contains(he.vertex)) {
	        		heTable.put(he.vertex, he);
	        	} else {
	        		Halfedge test = heTable.get(he.vertex);
    ...
    {
    	/*if (x1 == InfiniteVoronoi.minX || x2 == InfiniteVoronoi.minX
    	||  y1 == InfiniteVoronoi.minY || y2 == InfiniteVoronoi.minY) {
    		return;
    	}
    	
    	int mx = (InfiniteVoronoi.minX+(InfiniteVoronoi.scaledSize));
    	int my = (InfiniteVoronoi.minY+(InfiniteVoronoi.scaledSize));
        ...
        //r += 10;
        //leftSite.hull.addVertex(r,320);
        //leftSite.hull.addVertex(r+10,320+10);
        //if (leftSite != rightSite) {
        	leftSite.hull.addVertex(x1,y1);
        	leftSite.hull.addVertex(x2,y2);
        //}
        ...
        Cell c1 = cells.get(newEdge.site1);
		Cell c2 = cells.get(newEdge.site2);
		c1.edges.add(newEdge);
		c2.edges.add(newEdge);
		c1.x = (int)leftSite.coord.x;
		c1.y = (int)leftSite.coord.y;
		c2.x = (int)rightSite.coord.x;
		c2.y = (int)rightSite.coord.y;
    }

    private void clip_line(Edge e,Site store)
    {
    ...
    private void endpoint(Edge e, int lr, Site s,Site store)
    {
        e.ep[lr] = s;
        if (e.ep[RE - lr] == null)
        {
    ...
    private double dist(Site s, Site t)
    {
    	...
    	//return Math.abs(s.coord.x - t.coord.x) + Math.abs(s.coord.y - t.coord.y);
    }
    ...
    private boolean voronoi_bd()
    {
        ...
        while (true)
        {
            ...
            /* intersection is smallest - this is a vector event */
            {
                ...
                endpoint(lbnd.ELedge, lbnd.ELpm, v,newsite);
                // set the endpoint of
                // the left HalfEdge to be this vector
                endpoint(rbnd.ELedge, rbnd.ELpm, v,newsite);
                ...
                endpoint(e, RE - pm, v,newsite); // set one endpoint to the new edge
